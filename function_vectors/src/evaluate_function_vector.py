...
def run_evaluation(model, tokenizer, model_config, args, skip_shuffled=False):
    """
    Runs the full FV evaluation for a single (model, task) given an ALREADY-LOADED model.

    This is a pure orchestration extraction of the original __main__ body: the model is
    passed in (loaded once by the caller) instead of being loaded here, so a driver can
    process many tasks without reloading. The computation is otherwise byte-identical to
    the original script.

    skip_shuffled: if True, the 10-shot shuffled-label evaluation is skipped (used only as a
    budget fallback). Default False -> identical behavior/computation to the original script.
    """
...
if __name__ == "__main__":
    ...
    run_evaluation(model, tokenizer, model_config, args)
